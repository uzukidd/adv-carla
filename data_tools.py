...
from cudaext.ops.Rotated_IoU.oriented_iou_loss import cal_iou, cal_iou_3d
from pcdet.config import cfg, cfg_from_yaml_file
from pcdet.datasets import DatasetTemplate, KittiDataset, build_dataloader
from pcdet.datasets.processor.data_processor import DataProcessor
from pcdet.models import build_network, load_data_to_gpu
from pcdet.utils import common_utils
from pytorch3d.loss import mesh_laplacian_smoothing
from pytorch3d.ops import laplacian, sample_points_from_meshes
from pytorch3d.structures import Meshes, join_meshes_as_batch
from pytorch3d.transforms import Rotate, Scale, Translate, euler_angles_to_matrix
...
def register_data_processing(processing_name: str):
    def _real_decorator(processing_func):
        setattr(DataProcessor, processing_name, processing_func)
...
class kitti_carla_dataset(DatasetTemplate):
    def __init__(
        ...
        self.calib_path = Path(dataset_cfg.DATA_PATH) / Path(
            "generated/lidar_to_cam0.txt"
        )
    ...
    def prepare_predicted_gtboxes(
        ...
        with tqdm(
            ...
            for batch_dict in pbar:
                ...
                pred_labels: torch.Tensor = pred_dicts[0]["pred_labels"].view(
                    -1
                )  # [N, ]
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        for i, det_anno in enumerate(det_annos):
            ...
            # iou3d [M, N]
            iou2d, _, _, _ = cal_iou(
                box3d_gt_extended[..., [0, 1, 3, 4, 6]],
                box3d_bl_extended[..., [0, 1, 3, 4, 6]],
            )
    ...
    def prepare_kitti_det_annos(self, batch_index, box_dict):
        ...
        def get_template_prediction(num_samples):
            ret_dict = {
                ...
                "bbox": np.tile(np.array([0.0, 0.0, 0.0, 50.0]), (num_samples, 1)),
        ...
        # )
        loc_camera = np.pad(
            pred_boxes[:, 0:3], ((0, 0), (0, 1)), constant_values=1
        ).dot(self.tf_lidar_to_cam0.T)
        pred_dict["name"] = np.array(self.class_names)[pred_labels - 1]
        pred_dict["alpha"] = -np.arctan2(-pred_boxes[:, 1], pred_boxes[:, 0]) + 0.0
    ...
    def prepare_kitti_gt_annos(self, batch_index, gt_boxes, gt_labels):
        loc_camera = np.pad(
            gt_boxes[:, 0:3].cpu().numpy(), ((0, 0), (0, 1)), constant_values=1
        ).dot(self.tf_lidar_to_cam0.T)

        annotations = {}
        annotations["name"] = np.array(self.class_names)[
            gt_labels.cpu().numpy().reshape(-1).astype(int) - 1
        ]
        annotations["truncated"] = np.ones(gt_boxes.size(0)) * 0.0
        annotations["occluded"] = np.ones(gt_boxes.size(0)) * 0.0
        annotations["alpha"] = np.ones(gt_boxes.size(0)) * 0.0
        annotations["bbox"] = np.tile(
            np.array([0.0, 0.0, 0.0, 50.0]), (gt_boxes.size(0), 1)
        )
        ...
        annotations["score"] = np.ones(gt_boxes.size(0)) * 0.0
        annotations["difficulty"] = np.ones(gt_boxes.size(0)) * 1.0
...
class adv_dataset(DatasetTemplate):
    ...
    def __init__(
        ...
        if rooftop_annotate is not None:
            ...
            self.rooftop_approximate = self.load_annotated_rooftop(
                self.attack_config.ROOFTOP_ANNOTATE
            )
        ...
        if self.rooftop_approximate is not None:
            ...
            self.logger.info(
                "Successfully loaded rooftop appromximation: %d" % (rooftop_size)
            )

        self.universal_adv_patch_car = single_sphere(
            scale=self.car_adv_patch_scale, level=self.car_adv_patch_level
        )
    ...
    def prepare_predicted_gtboxes(
        ...
        with tqdm(
            ...
            for batch_dict in pbar:
                ...
                if (
                    batch_dict.get("points_with_semantic") is not None
                    and batch_dict.get("points_with_semantic")[0].shape[1] == 5
                ):
                    points_with_semantic = batch_dict["points_with_semantic"][0]
                    points_assign = points_in_boxes_gpu(
                        points_with_semantic[:, :3].unsqueeze(0),
                        preds_boxes.unsqueeze(0),
                    )
    ...
    def load_adversarial_parameter(self, path: str):
        ...
        self.universal_adv_patch_car.load_parameter(
            input_dict["universal_adv_patch_car"]
        )
    ...
    def __getitem__(self, index, adversarial_parameters: list[torch.Tensor] = None):
        ...
        if self.enabled_adversarial_patch:
            batch_dict = self.prepare_car_gtbox(batch_dict, rooftop_approximate)
            batch_dict = self.prepare_pedestrain_gtbox(batch_dict)
            batch_dict = self.prepare_bicycle_gtbox(batch_dict)

            batch_dict = self.prepare_adversarial_data(
                batch_dict, adversarial_parameters
            )

            # batch_dict = self.collect_all_class_gtbox(batch_dict)
        batch_dict = self.data_processor.forward(data_dict=batch_dict)
        load_data_to_gpu(batch_dict)
    ...
    def get_gt_boxes_statistic_info(self):
        ...
        for batch_dict in self.parent_dataset:
            classes, counts = np.unique(
                batch_dict["gt_boxes"][:, 7], return_counts=True
            )
    ...
    def prepare_adversarial_data(
        ...
        if gt_boxes_car is not None and self.enable_car:
            ...
            batch_dict["points"] = self.attach_adv_patch_scene_car_aux(
                batch_dict["points"],
    ...
    def prepare_car_gtbox(self, batch_dict, rooftop_approximate: list[np.ndarray]):
        """
        gt_boxes: [N, 8]
        """
        gt_boxes: torch.Tensor = batch_dict["gt_boxes"]
    ...
    @staticmethod
    def generate_basic_mesh(level: int = 0, scale: list = [0.7, 0.7, 0.5], eps=0.0):
        ...
        if rooftop_approximate is not None:
            ...
            for i in range(n):
                ...
                if self.lidar is not None:
                    ...
                    pts = adv_patch.sample_points(sample_amount=sample_amount).view(
                        -1, 3
                    ) - adv_patch.get_base_coord(False)
                    extend_pts = torch.from_numpy(rooftop_approximate[i])[
                        None, :3
                    ].float().cuda() + adv_dataset.rotate_points(pts, theta[i])
    ...
    def attach_adv_patch_scene_ped_aux(
        self,
        points,
        adv_patch: adversarial_patch_3d,
        ...
        for i in range(n):
            pts = (
                adv_patch.sample_points(sample_amount=sample_amount).view(-1, 3)
                - adv_patch.base_coord
            )
            ...
            try:
                if key in ["voxels", "voxel_num_points"]:
                    ...
                    batch_gt_boxes3d = val[0].new_zeros(
                        (batch_size, max_gt, val[0].size(-1))
                    )
